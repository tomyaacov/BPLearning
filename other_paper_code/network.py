...
class NeuralNetwork:
    ...
    def __init__(self):
        ...
        for to in range(NUMBER_OF_POSITIONS):
            self.adder[to] = 1.0 * (to + 1) / (NUMBER_OF_POSITIONS) ### weird


    def check(self, word): # get a word output - forward propogation
            ...
            for i in range(NUMBER_OF_CHARS):
                ...
                for k in range(1, NUMBER_OF_POSITIONS):
                    if (nn.tensor[k][i][j] > nn.tensor[max_ind][i][j]):
                        max_ind = k
                print(str(j) + ' -> ' + str(max_ind) + ' [label="' + str(DICTIONARY[i]) + '"]')
        for k in range(NUMBER_OF_POSITIONS):
            if (nn.adder[k] > 0.5):
                print(str(k) + " [style=filled, fillcolor=red]")
